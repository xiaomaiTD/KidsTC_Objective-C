...
- (void)handleLoginSucceedWithOpenId:(NSString *)openId accessToken:(NSString *)token loginType:(ThirdPartyLoginType)type {
    ...
    if ([openId length] > 0 && [token length] > 0) {
        ...
        [Request startWithName:@"THIRD_LOGIN" param:param progress:nil success:^(NSURLSessionDataTask *task, NSDictionary *dic) {
            if (self.successBlock) self.successBlock(dic);
