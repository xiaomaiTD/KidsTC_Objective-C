...
static NSString *const kWeChatLoginIdentifier = @"kWeChatLoginIdentifier";
...
+ (PayReq *)payRequestFromPayInfo:(PayInfo *)info{
    if (info.payType != PayTypeWeChat) return nil;
        ...
        failure:(void (^)(NSError *))failure
{
    ...
    PayReq *request = [WeChatManager payRequestFromPayInfo:info];
    return [WXApi sendReq:request];
}
