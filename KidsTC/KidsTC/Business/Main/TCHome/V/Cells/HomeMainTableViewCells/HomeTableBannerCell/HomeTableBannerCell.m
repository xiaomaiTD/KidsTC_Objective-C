...
static NSString *const ID = @"HomeTableBannerCollectionCell";
static int const kHomeTableBannerCellMaxSections = 11;

@interface HomeTableBannerCell ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation HomeTableBannerCell

- (void)awakeFromNib {
    [super awakeFromNib];

    [self.collectionView registerNib:[UINib nibWithNibName:@"HomeTableBannerCollectionCell" bundle:nil] forCellWithReuseIdentifier:ID];
...
- (NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView {
    return _narrowImg.count>1?kHomeTableBannerCellMaxSections:1;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    HomeTableBannerCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
...
- (void)nextPage
{
    ...
    if (nextItem<count && nextSection<kHomeTableBannerCellMaxSections) {
...
- (NSIndexPath *)resetIndexPath
{
    ...
    // 马上显示回最中间那组的数据
    NSIndexPath *currentIndexPathReset = [NSIndexPath indexPathForItem:currentIndexPath.item inSection:kHomeTableBannerCellMaxSections/2];
