...
- (void)configWithModel:(CommentListItemModel *)model {
    ...
    if (model) {
        [self.faceImageView sd_setImageWithURL:model.faceImageUrl];
...
- (void)buildPhotosViewWithPhotoUrlStrings:(NSArray<NSString *> *)urlStrings {
    ...
    if (sameDataSource) {
        ...
        for (NSString *string in urlStrings) {
            ...
            //异步加载图片
            [imageView sd_setImageWithURL:[NSURL URLWithString:string] placeholderImage:PLACEHOLDERIMAGE_BIG completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                //原始图片
