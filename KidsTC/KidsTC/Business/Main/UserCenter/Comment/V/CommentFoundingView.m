...
- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder
{
    ...
    {
        bLoad = YES;
        CommentFoundingView *view = [ToolBox getObjectFromNibWithView:self];
        [view buildSubviews];
        return view;
    }
    bLoad = NO;
...
- (void)buildSubviews {
    ...
    [self.submitButton setBackgroundColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        case 0:{
            [self.serviceImageView sd_setImageWithURL:self.commentModel.imageUrl placeholderImage:PLACEHOLDERIMAGE_SMALL];
...
- (void)textViewDidChange:(UITextView *)textView {
    ...
    self.commentLengthLabel.text = [NSString stringWithFormat:@"%zd",number];
}
