...
@property (nonatomic, assign) BOOL isGetMoreData;
...
- (void)startUpdateDataWithType:(KTCCommentType)type {
    ...
    [self.commentManager loadCommentsWithIdentifier:self.identifier RequestParam:param succeed:^(NSDictionary *data) {
...
- (void)getMoreDataWithType:(KTCCommentType)type {
    ...
    __weak CommentListViewModel *weakSelf = self;
    [self.commentManager loadCommentsWithIdentifier:self.identifier RequestParam:param succeed:^(NSDictionary *data) {
        CommentListViewModel *strongSelf = weakSelf;
        [strongSelf loadCommentsSucceedWithData:data segmmentTag:strongSelf.currentTag];
    } failure:^(NSError *error) {
        CommentListViewModel *strongSelf = weakSelf;
        [strongSelf loadCommentsFailedWithError:error segmmentTag:strongSelf.currentTag];
    }];
}
