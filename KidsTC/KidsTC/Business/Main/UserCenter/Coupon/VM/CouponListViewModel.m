...
- (void)startUpdateDataWithViewTag:(CouponListViewTag)tag {
    ...
    [Request startWithName:@"COUPON_QUERRY" param:param progress:nil success:^(NSURLSessionDataTask *task, NSDictionary *dic) {
        [self loadCouponsSucceedWithData:dic segmmentTag:self.currentTag];
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        [self loadCouponsFailedWithError:error segmmentTag:self.currentTag];
    }];
...
- (void)getMoreDataWithViewTag:(CouponListViewTag)tag {
    ...
    NSDictionary *param = [NSDictionary dictionaryWithObjectsAndKeys:
                           ...
                           [NSNumber numberWithInteger:PageSize], @"pagecount", nil];
    [Request startWithName:@"COUPON_QUERRY" param:param progress:nil success:^(NSURLSessionDataTask *task, NSDictionary *dic) {
        [self loadMoreCouponsSucceedWithData:dic segmmentTag:self.currentTag];
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        [self loadMoreCouponsFailedWithError:error segmmentTag:self.currentTag];
    }];
}
