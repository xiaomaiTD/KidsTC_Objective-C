...
- (void)awakeFromNib {
    ...
    self.serviceStatusImageView.layer.cornerRadius = 5;
    self.serviceStatusImageView.layer.masksToBounds = YES;
    
    [self.saledCountLabel setTextColor:COLOR_PINK];
...
- (void)configWithItemModel:(ServiceListItemModel *)model {
    ...
    if (model.imageUrl) {
        [self.serviceImageView sd_setImageWithURL:model.imageUrl placeholderImage:PLACEHOLDERIMAGE_BIG_LOG];
    }
