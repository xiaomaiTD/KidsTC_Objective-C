...
- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder
{
    ...
    {
        bLoad = YES;
        AppointmentOrderListView *view = [ToolBox getObjectFromNibWithView:self];
        [view buildSubviews];
        return view;
    }
    bLoad = NO;
...
- (void)buildSubviews {
    ...
    RefreshFooter *mj_footer = [RefreshFooter footerWithRefreshingBlock:^{
        StrongSelf(self)
        BOOL noMore = [[self.noMoreDataDic objectForKey:[NSString stringWithFormat:@"%d", self.currentLsitStatus]] boolValue];
        if (noMore) {
