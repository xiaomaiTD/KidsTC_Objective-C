...
- (void)reloadFavourateViewWithData:(NSDictionary *)data forSegmmentTag:(FavourateViewSegmentTag)tag {
    ...
    if ([dataArray isKindOfClass:[NSArray class]] && [dataArray count] > 0) {
        ...
        switch (tag) {
            ...
            case FavourateViewSegmentTagNews:
            {
                ArticleCollectionResponse *articleCollectionResponse = [ArticleCollectionResponse modelWithDictionary:data];
                //list
                NSArray *list = articleCollectionResponse.data;
                for (ALstItem *item in list) {
                    ArticleLayout *layout = [[ArticleLayout alloc]init];
                    layout.item = item;
                    [self.newsResultArray addObject:layout];
                }
...
- (void)deleteFavourateDataForTag:(FavourateViewSegmentTag)tag atInde:(NSUInteger)index {
    ...
    switch (tag) {
        ...
        {
            type = KTCFavouriteTypeNews;
            ArticleLayout *layout = [self.newsResultArray objectAtIndex:index];
            ALstItem *lstItem = layout.item;
            NSString *sysNo = [lstItem.articleSysNo isNotNull]?lstItem.articleSysNo:@"";
            NSDictionary *parameters = @{@"relationSysNo":sysNo,
                                         @"likeType":@"2",
                                         @"isLike":@"0"};
    ...
    if (tag!=FavourateViewSegmentTagNews) {
        __weak FavourateViewModel *weakSelf = self;
