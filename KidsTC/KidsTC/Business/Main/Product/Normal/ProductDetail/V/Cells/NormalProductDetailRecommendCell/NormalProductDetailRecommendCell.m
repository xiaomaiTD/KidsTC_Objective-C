//
//  NormalProductDetailRecommendCell.m
//  KidsTC
//
...
@interface NormalProductDetailRecommendCell ()
...
@implementation NormalProductDetailRecommendCell
...
- (void)tapAction:(UITapGestureRecognizer *)tapGR {
    if (_index<self.data.recommends.count) {
        if ([self.delegate respondsToSelector:@selector(normalProductDetailBaseCell:actionType:value:)]) {
            [self.delegate normalProductDetailBaseCell:self actionType:NormalProductDetailBaseCellActionTypeSegue value:self.data.recommends[_index].segueModel];
        }
    }
}
