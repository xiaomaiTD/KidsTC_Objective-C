//
//  NormalProductDetailStandardCell.m
//  KidsTC
//
...
@interface NormalProductDetailStandardCell ()
...
@implementation NormalProductDetailStandardCell
...
- (void)tapAction:(UITapGestureRecognizer *)tapGR {
    if (_index<self.data.product_standards.count) {
        if ([self.delegate respondsToSelector:@selector(normalProductDetailBaseCell:actionType:value:)]) {
            [self.delegate normalProductDetailBaseCell:self actionType:NormalProductDetailBaseCellActionTypeStandard value:self.data.product_standards[_index]];
        }
    }
}
...
- (IBAction)btnAction:(UIButton *)sender {
    if (_index<self.data.product_standards.count) {
        if ([self.delegate respondsToSelector:@selector(normalProductDetailBaseCell:actionType:value:)]) {
            [self.delegate normalProductDetailBaseCell:self actionType:NormalProductDetailBaseCellActionTypeBuyStandard value:self.data.product_standards[_index]];
        }
    }
}
@end
