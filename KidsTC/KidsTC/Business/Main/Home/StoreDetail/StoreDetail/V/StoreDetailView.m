...
@property (weak, nonatomic) IBOutlet UIImageView *storeAddressBriefImageView;
...
- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder
{
    ...
    {
        bLoad = YES;
        StoreDetailView *view = [ToolBox getObjectFromNibWithView:self];
        [view buildSubviews];
        return view;
    }
    bLoad = NO;
...
- (void)buildSubviews {
    ...
    [ToolBox renderGradientForView:self.storeAddressBriefAlphaView displayFrame:CGRectMake(0, 0, SCREEN_WIDTH, self.storeAddressBriefAlphaView.frame.size.height) startPoint:CGPointMake(0.5, 0) endPoint:CGPointMake(0.5, 1) colors:[NSArray arrayWithObjects:RGBA(0, 0, 0, 0), RGBA(0, 0, 0, 1), nil] locations:nil];
...
//    self.detailInfoView.scrollView.scrollEnabled = NO;
    ...
    [self.nearbyBGView setBackgroundColor:COLOR_BG_CEll];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.contentView setBackgroundColor:COLOR_BG_CEll];
    return cell;
}
...
- (void)configTopCell {
    ...
    if ([self.detailModel.storeAddress length] > 0) {
        ...
        self.storeAddressBriefImageView.image = [UIImage imageNamed:@"located"];
    } else {
        [self.storeAddressBriefBGView setHidden:YES];
        self.storeAddressBriefImageView.image = [UIImage imageNamed:@"unlocated"];
    }
    ...
    NSDictionary *fontAttribute = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:13], NSFontAttributeName, COLOR_PINK, NSForegroundColorAttributeName, nil];
...
- (void)configRecommendCell {
    ...
    [self.recommenderFaceImageView sd_setImageWithURL:self.detailModel.recommenderFaceImageUrl];
