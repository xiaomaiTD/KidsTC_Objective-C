...
- (BOOL)fillWithRawData:(NSDictionary *)data {
    ...
    if ([imgUrls isKindOfClass:[NSArray class]]) {
        ...
        for (NSString *urlString in imgUrls) {
            if (urlString &&
                ![urlString isKindOfClass:[NSNull class]]) {
    ...
    if ([location isKindOfClass:[NSDictionary class]]) {
        ...
        self.storeCoordinate = [ToolBox coordinateFromString:[location objectForKey:@"mapAddr"]];
    }
    //activity
...
- (CGFloat)topCellHeight {
    ...
    height += 30;
//    height += [ToolBox heightForLabelWithWidth:SCREEN_WIDTH - 20 LineBreakMode:NSLineBreakByCharWrapping Font:[UIFont systemFontOfSize:13] topGap:10 bottomGap:10 maxLine:0 andText:self.storeBrief];
    height += [ToolBox heightForAttributeLabelWithWidth:SCREEN_WIDTH - 20 LineBreakMode:NSLineBreakByCharWrapping Font:[UIFont systemFontOfSize:15] topGap:10 bottomGap:10 maxLine:0 andText:self.storeBrief];
    height += 25;
    return height;
}
...
- (CGFloat)briefCellHeight {
    return [ToolBox heightForLabelWithWidth:SCREEN_WIDTH - 10 LineBreakMode:NSLineBreakByCharWrapping Font:[UIFont systemFontOfSize:13] topGap:10 bottomGap:10 maxLine:0 andText:self.storeBrief];
}
