...
- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder
{
    ...
    {
        bLoad = YES;
        ActivityFilterView *view = [ToolBox getObjectFromNibWithView:self];
        return view;
    }
    bLoad = NO;
    return self;
...
- (void)buildSubviews {
    ...
    [self.filterBGView setBackgroundColor:[UIColor whiteColor]];
    
    self.areaTable.backgroundView = nil;
    self.areaTable.backgroundColor = [UIColor groupTableViewBackgroundColor];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [bgView setBackgroundColor:COLOR_PINK];
    ...
    if ([self.categoryButtonsArray count] > index) {
        for (NSInteger buttonIndex = 0; buttonIndex < [self.categoryButtonsArray count]; buttonIndex ++) {
            ...
            if (buttonIndex == index) {
                ...
                [button.layer setBorderColor:COLOR_PINK.CGColor];
...
- (void)buildCategoryView {
    ...
    for (NSInteger index = 0; index < [self.categoryNameArray count]; index ++) {
        ...
        [button setTitleColor:COLOR_PINK forState:UIControlStateSelected];
