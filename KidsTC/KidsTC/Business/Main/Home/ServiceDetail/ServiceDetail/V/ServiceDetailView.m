...
#define MealImageSize 12

@interface MealButton : UIButton

@end

@implementation MealButton

- (CGRect)imageRectForContentRect:(CGRect)contentRect{
    CGFloat y = (CGRectGetHeight(contentRect) - MealImageSize)*0.5;
    return CGRectMake(CGRectGetWidth(contentRect)-MealImageSize, y, MealImageSize, MealImageSize);
}
- (CGRect)titleRectForContentRect:(CGRect)contentRect{
    return CGRectMake(0, 0, CGRectGetWidth(contentRect)-MealImageSize, CGRectGetHeight(contentRect));
}
...
typedef enum {
    ...
    ServiceDetailTableCellTagMealTitle = 7,
    ServiceDetailTableCellTagMeal = 8,
    ServiceDetailTableCellTagRecommend = 9,
    ServiceDetailTableCellTagActivity = 10,
    ServiceDetailTableCellTagMoreServiceTitle = 11,
    ServiceDetailTableCellTagMoreService = 12
...
@property (strong, nonatomic) AUISegmentView *segmentView;
@property (strong, nonatomic) ServiceDetailMoreInfoView *moreInfoView;
...
@property (strong, nonatomic) IBOutlet UITableViewCell *mealTitleCell;
@property (strong, nonatomic) IBOutlet UITableViewCell *mealCell;
...
//meal
@property (weak, nonatomic) IBOutlet UIView *mealTitleCellTagView;
@property (weak, nonatomic) IBOutlet UILabel *mealTitleLabel;
@property (weak, nonatomic) IBOutlet UILabel *mealProductNameLabel;
@property (weak, nonatomic) IBOutlet MealButton *mealSelectBtn;
...
- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder
{
    ...
    {
        bLoad = YES;
        ServiceDetailView *view = [ToolBox getObjectFromNibWithView:self];
        [view buildSubviews];
        return view;
    }
    bLoad = NO;
...
- (void)buildSubviews {
    ...
    [ToolBox renderGradientForView:self.storeBriefAlphaView displayFrame:CGRectMake(0, 0, SCREEN_WIDTH, self.storeBriefAlphaView.frame.size.height) startPoint:CGPointMake(0.5, 0) endPoint:CGPointMake(0.5, 1) colors:[NSArray arrayWithObjects:RGBA(0, 0, 0, 0), RGBA(0, 0, 0, 1), nil] locations:nil];
    ...
    [self.priceDescriptionLabel setTextColor:COLOR_PINK];
    //segment view
    //moreinfo view
    //cells
    [self.topCell.contentView setBackgroundColor:COLOR_BG_CEll];
    
    [self.priceCell.contentView setBackgroundColor:COLOR_BG_CEll];
    
    [self.InsuranceView setFontSize:13];
    [self.InsuranceCell.contentView setBackgroundColor:COLOR_BG_CEll];
    
    [self.couponCell.contentView setBackgroundColor:COLOR_BG_CEll];
    
    [self.contentCell.contentView setBackgroundColor:COLOR_BG_CEll];
    
    [self.noticeTitleCell.contentView setBackgroundColor:COLOR_BG_CEll];
    [self.noticeTitleLabel setText:@"购买须知"];
    [self.noticeTitleCellTagView setBackgroundColor:COLOR_PINK];
    [self.noticeCell.contentView setBackgroundColor:COLOR_BG_CEll];
    
    [self.mealTitleCell.contentView setBackgroundColor:COLOR_BG_CEll];
    [self.mealTitleLabel setText:@"已选套餐"];
    [self.mealTitleCellTagView setBackgroundColor:COLOR_PINK];
    [self.mealCell.contentView setBackgroundColor:COLOR_BG_CEll];
    
    [self.recommendCell.contentView setBackgroundColor:COLOR_BG_CEll];
    self.recommendFaceImageView.layer.cornerRadius = 30;
    self.recommendFaceImageView.layer.masksToBounds = YES;
    
    [self.moreServiceTitleCellTagView setBackgroundColor:COLOR_PINK];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (cell.tag) {
        ...
        case ServiceDetailTableCellTagNotice:
        {
            ...
            break;
        case ServiceDetailTableCellTagMealTitle:
        {
            [self configMealTitleCell];
        }
            break;
        case ServiceDetailTableCellTagMeal:
        {
            [self configMealCell];
        }
            break;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (cell.tag) {
        ...
        case ServiceDetailTableCellTagMealTitle:
        {
            height = [self.detailModel mealTitleCellHeight];
        }
            break;
        case ServiceDetailTableCellTagMeal:
        {
            height = [self.detailModel mealCellHeight];
        }
            break;
        case ServiceDetailTableCellTagRecommend:
        {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView == self.scrollView) {
        CGFloat offset = self.scrollView.contentOffset.y+64;
...
- (UIImageView *)bannerImageViewOnScrollView:(AUIBannerScrollView *)scrollView withViewFrame:(CGRect)frame atIndex:(NSUInteger)index {
    ...
    [imageView sd_setImageWithURL:imageUrl placeholderImage:PLACEHOLDERIMAGE_BIG];
    return imageView;
}
...
- (void)initSegmentView {
    ...
    CGFloat offset = self.scrollView.contentOffset.y+64;
    CGFloat segmentViewY = offset>=self.tableViewHeight?offset:self.tableViewHeight;
    [self.segmentView setFrame:CGRectMake(0, segmentViewY, SCREEN_WIDTH, SegmentViewHeight)];
}
...
- (void)configTopCell {
    ...
    [ToolBox resetLineView:self.bannerScrollView withLayoutAttribute:NSLayoutAttributeHeight constant:self.detailModel.imageRatio * SCREEN_WIDTH];
    //store brief
    ...
    if (storeCount > 1) {
        ...
        if (itemModel.location) {
            ...
            self.locationImageView.image = [UIImage imageNamed:@"located"];
        } else {
            [self.storeBriefAlphaView setHidden:YES];
            [self.storeBriefBGView setHidden:YES];
            self.locationImageView.image = [UIImage imageNamed:@"unlocated"];
        }
    }
    //others
    ...
    NSDictionary *fontAttribute = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:15], NSFontAttributeName, COLOR_PINK, NSForegroundColorAttributeName, nil];
...
- (void)configNoticeCell {
    ...
    [self.noticeBGView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    for (ServiceDetailNoticeItem *item in self.detailModel.noticeArray) {
        ...
        xPosition = margin;
        yPosition += contentLabel.frame.size.height + margin;
    }
}

- (void)configMealTitleCell {
    
}

- (void)configMealCell {
    if (self.detailModel.currentProductStandardsItem) {
        NSString *productName = self.detailModel.currentProductStandardsItem.productName;
        NSString *standardName = self.detailModel.currentProductStandardsItem.standardName;
        self.mealProductNameLabel.text = [NSString stringWithFormat:@"%@,%@",productName,standardName];
    }
}

- (void)configRecommendCell {
    
    [self.recommendFaceImageView sd_setImageWithURL:self.detailModel.recommenderFaceImageUrl];
...
- (IBAction)mealSelectAction:(MealButton *)sender {
    if ([self.delegate respondsToSelector:@selector(serviceDetailViewDidClickSeleteMeal:)]) {
        [self.delegate serviceDetailViewDidClickSeleteMeal:self];
    }
}
...
- (void)reloadData {
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(detailModelForServiceDetailView:)]) {
        ...
        if (self.detailModel) {
            ...
            if (self.detailModel.currentProductStandardsItem) {
                NSArray *section5 = @[self.mealTitleCell,self.mealCell];
                [self.cellArray addObject:section5];
                self.tableViewHeight += SectionGap;
                self.tableViewHeight += [self.detailModel mealTitleCellHeight]+ [self.detailModel mealCellHeight];
            }
            
            if ([self.detailModel.recommendString length] > 0) {
                NSArray *section6 = [NSArray arrayWithObject:self.recommendCell];
            ...
            if ([self.detailModel.moreServiceItems count] > 0) {
                NSMutableArray *section7 = [NSMutableArray arrayWithObject:self.moreServiceTitleCell];
                self.tableViewHeight += SectionGap;
