...
- (void)didClickedImage:(id)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(BannerScrollView:didClickedAtIndex:)]) {
        [self.delegate BannerScrollView:self didClickedAtIndex:index];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView == self.scrollView) {
        ...
        if (!self.recyclable) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(BannerScrollView:didScrolledToIndex:)]) {
                [self.delegate BannerScrollView:self didScrolledToIndex:index];
            }
        }
    }
}
...
- (void)resetImageForIndex:(NSUInteger)iIndex withPageIndex:(NSUInteger)pIndex {
    if (self.dataSource) {
        ...
        if (self.imageViewsArray.count>pIndex) {
