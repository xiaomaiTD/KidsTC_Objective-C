...
@interface NoticeScrollView () <UIScrollViewDelegate>
...
- (void)didClickedLabel:(id)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(NoticeView:didClickedAtIndex:)]) {
        [self.delegate NoticeView:self didClickedAtIndex:index];
    }
}
...
- (void)resetCurrentPageIndex:(NSUInteger)index {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(NoticeView:didScrolledToIndex:)]) {
        [self.delegate NoticeView:self didScrolledToIndex:index];
    }
}
