...
#define cellIdentifier @"ArticleAutoRollCell"
#define AutoRollViewMaxSections 11

@interface ArticleAutoRollView()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation ArticleAutoRollView
...
-(instancetype)initWithFrame:(CGRect)frame items:(NSArray *)items delegate:(id<ArticleAutoRollViewDelegate>)delegate isBannerHaveInset:(BOOL)isBannerHaveInset{
    ...
    if (self) {
        ...
        self.isBannerHaveInset = isBannerHaveInset;
        ...
        [self.collectionView registerClass:[ArticleAutoRollCell class] forCellWithReuseIdentifier:cellIdentifier];
        
        
        self.pageContol=[[UIPageControl alloc]init];
        if (self.isBannerHaveInset) {
            self.pageContol.frame = CGRectMake(0, CGRectGetHeight(self.bounds)-40, CGRectGetWidth(self.bounds), 40);
        }else{
            self.pageContol.frame = CGRectMake(0, CGRectGetHeight(self.bounds)-20, CGRectGetWidth(self.bounds), 20);
        }
        UIColor *color = COLOR_PINK;
        self.pageContol.currentPageIndicatorTintColor = color;
        self.pageContol.pageIndicatorTintColor = [UIColor colorWithWhite:0 alpha:0.2];
        self.pageContol.numberOfPages = self.items.count;
        [self addSubview:self.pageContol];
        
        
        // 默认显示最中间的那组
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:AutoRollViewMaxSections/2] atScrollPosition:UICollectionViewScrollPositionLeft animated:NO];
...
-(void)setItems:(NSArray *)items{
    _items = items;
    self.pageContol.numberOfPages = _items.count;
...
{
    ArticleAutoRollCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
    
    cell.bannerItem = self.items[indexPath.row];
    cell.isBannerHaveInset = self.isBannerHaveInset;
    return cell;
}
