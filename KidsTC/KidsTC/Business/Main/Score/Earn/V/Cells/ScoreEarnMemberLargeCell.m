...
@interface ScoreEarnMemberLargeCell ()
...
@implementation ScoreEarnMemberLargeCell
...
- (void)setItem:(ScoreEarnShowItem *)item {
    ...
    ScoreProductItem *product = products[index];
...
- (void)tapAction {
    if ([self.delegate respondsToSelector:@selector(scoreEarnBaseCell:actionType:value:)]) {
        [self.delegate scoreEarnBaseCell:self actionType:ScoreEarnBaseCellActionTypeSegue value:self.product.segueModel];
    }
}
@end
