//
//  RadishOrderDetailDeliverCell.m
//  KidsTC
//
...
@interface RadishOrderDetailDeliverCell ()
...
@implementation RadishOrderDetailDeliverCell
...
- (void)setData:(RadishOrderDetailData *)data {
    [super setData:data];
    RadishOrderDetailDeliver *deliver = self.data.deliver;
...
- (void)settupDeliverInfo {
    ...
    [self.data.deliver.items enumerateObjectsUsingBlock:^(RadishOrderDetailDeliverItem *obj, NSUInteger idx, BOOL *stop) {
        StrongSelf(self)
        ...
        [attDeliverInfo setTextHighlightRange:range
                              ...
                              backgroundColor:[UIColor colorWithWhite:0.000 alpha:0.220]
                                    tapAction:^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect){
                                        if (![self.delegate respondsToSelector:@selector(radishOrderDetailBaseCell:actionType:value:)]) return;
                                        if (obj.isCall) {
                                            if (![obj.value isNotNull]) return;
                                            [self.delegate radishOrderDetailBaseCell:self actionType:RadishOrderDetailBaseCellActionTypeDeliberCall value:obj.value];
                                        }else{
                                            [self.delegate radishOrderDetailBaseCell:self actionType:RadishOrderDetailBaseCellActionTypeSegue value:obj.segueModel];
                                        }
                                    }];
    }];
