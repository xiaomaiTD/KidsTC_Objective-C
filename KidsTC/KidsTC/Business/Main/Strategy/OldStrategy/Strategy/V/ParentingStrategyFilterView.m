...
- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder
{
    ...
    {
        bLoad = YES;
        ParentingStrategyFilterView *view = [ToolBox getObjectFromNibWithView:self];
        return view;
    }
    bLoad = NO;
    return self;
...
- (void)buildSubviews {
    [self.headerBar setBackgroundColor:COLOR_PINK];
    [self.headerLabel setTextColor:[UIColor whiteColor]];
    [self.filterBGView setBackgroundColor:[UIColor groupTableViewBackgroundColor]];
...
//    self.areaTable.tableHeaderView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 0.01)];
    ...
    [self.hotSortButton setBackgroundColor:COLOR_BG_CEll forState:UIControlStateNormal];
    [self.hotSortButton setBackgroundColor:COLOR_BG_CEll forState:UIControlStateSelected];
    [self.hotSortButton setBackgroundColor:COLOR_BG_CEll forState:UIControlStateSelected];
    
    [self.timeSortButton setBackgroundColor:COLOR_BG_CEll forState:UIControlStateNormal];
    [self.timeSortButton setBackgroundColor:COLOR_BG_CEll forState:UIControlStateSelected];
    [self.timeSortButton setBackgroundColor:COLOR_BG_CEll forState:UIControlStateSelected];
}
...
- (void)showWithSelectedSortType:(ParentingStrategySortType)type selectedAreaIndex:(NSUInteger)index {
    ...
    __block NSMutableArray<NSString *> *AddrNames = [NSMutableArray array];
    [Addr enumerateObjectsUsingBlock:^(AreaAndAgeListItem *obj, NSUInteger idx, BOOL *stop) {
        if (obj.Name.length>0) [AddrNames addObject:obj.Name];
    }];
    [self setAreaNameArray:AddrNames];
