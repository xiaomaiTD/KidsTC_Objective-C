...
- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder
{
    ...
    {
        bLoad = YES;
        ParentingStrategyDetailView *view = [ToolBox getObjectFromNibWithView:self];
        [view buildSubviews];
        return view;
    }
    bLoad = NO;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section < [self.detailModel.cellModels count]) {
        ...
        [bottomLineView setBackgroundColor:COLOR_PINK];
        ...
        if ([model.title length] > 0 || section == 0) {
            ...
            [circleView setBackgroundColor:[UIColor groupTableViewBackgroundColor]];
            circleView.layer.borderColor = COLOR_PINK.CGColor;
        ...
        if (section > 0) {
            ...
            [topLineView setBackgroundColor:COLOR_PINK];
...
- (void)parentingStrategyDetailViewCell:(ParentingStrategyDetailViewCell *)cell didSelectedLinkWithSegueModel:(SegueModel *)model {
...
- (UIView *)buildHeaderView {
    ...
    [self.mainImageView sd_setImageWithURL:self.detailModel.mainImageUrl placeholderImage:PLACEHOLDERIMAGE_BIG];
    ...
    if (strategyDescriptionViewHeight > 0) {
        ...
        NSDictionary *fontAttribute = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:14], NSFontAttributeName, COLOR_TEXT, NSForegroundColorAttributeName, nil];
        [labelString setAttributes:fontAttribute range:NSMakeRange(0, [wholeString length])];
        NSDictionary *attribute = [NSDictionary dictionaryWithObjectsAndKeys:COLOR_PINK, NSForegroundColorAttributeName, nil];
        ...
        [self.descriptionLabel setFrame:CGRectMake(0, 10, SCREEN_WIDTH - 40, strategyDescriptionViewHeight-20)];
...
- (void)buildTagView {
    ...
    for (NSUInteger index = 0; index < [self.detailModel.tagNames count]; index ++) {
        ...
        //加点边距
        [button setFrame:CGRectMake(xPosition, yPosition, button.frame.size.width + buttonHeight, buttonHeight)];
        
        [button.layer setCornerRadius:buttonHeight*0.5];
        [button.layer setBorderWidth:2/[UIScreen mainScreen].scale];
        [button.layer setBorderColor:COLOR_BLUE.CGColor];
